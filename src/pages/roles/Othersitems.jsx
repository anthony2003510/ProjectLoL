...
function Othersitems() {
  ...
  function getTagRole(tag)
  {
    let result = false
    tag.forEach((tagrole) => {
        if(tagrole == "Mana" || tagrole == "Health" || tagrole == "ManaRegen" || tagrole == "Boots")
        {
          result = true;
        }
    });
    ...
    <div>
      <h1>OTHER ITEMS</h1>
        ...
        itemsValues?.map((item,i)=>(
          ...
          (
            <div key={i}>
              <div style={{height: "225px"}} className="flex flex-row items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-[#091428] dark:hover:bg-gray-800">
